...
class RM_Chronos_Toolkit {
    ...
    public static function get_merged_email($sub_id, $template, $user_id = null) {
        ...
        $sub_data = $wpdb->get_var("SELECT `data` FROM {$subs_table} WHERE `submission_id` = $sub_id");
    ...
    public static function get_already_processed_subs($task_id, $action) {
        ...
        $res = $wpdb->get_row("SELECT * FROM {$task_exe_log_table} WHERE `task_id` = {$task_id} AND `action` = '{$action}'");
